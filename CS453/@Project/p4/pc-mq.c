...
int main(int argc, char **argv)
{
    ...
    fprintf(stderr, "%d queues %d producers %d consumers %d items %d pool size\n",
			num_queue, num_producers, num_consumers, maxcount*num_producers, poolsize);
	// pool = tsb_createList(compareToItem, toStringItem, freeItem, poolsize);

    pool = (tsbListPtr *) malloc(sizeof(tsbListPtr) * num_queue);
...
void *producer(void *ptr)
{
	...
	while (i < maxcount) 
    {
        ...
        while(tsb_getSize(pool[index % num_queue]) == poolsize)
        {
            index++;
        }
...
void *consumer(void *ptr)
{
    ...
    int choice;
    int thread_number;
    pthread_t mytid = pthread_self();/* print item using toString method */
    char *itemString;
    ...
    while(TRUE)
    {
        if(tsb_getSize(pool[index % num_queue]) == 0)
        {
            index++;
        }
		...
		if (choice) 
        {
			node = tsb_removeFront(pool[index % num_queue]);
		} 
        else 
        {
			node = tsb_removeRear(pool[index % num_queue]);
		}
