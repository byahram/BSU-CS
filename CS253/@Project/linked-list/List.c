...
void addAtRear(struct list* list, struct node* node)
{
	...
	// Update to add at rear
	node -> next = list -> tail;
	node -> prev = NULL;

	// If the list is empty then we need to set BOTH the head
	// and tail to point to the same list
	if(list -> head == NULL)
	{
		list -> head = node;
		list -> tail = node;
	...
	{
		// Probably should look at this code.
		// Or we can just let if fly and test it in production.
		list -> tail -> next = node;
		list -> tail = node;
	}
}
