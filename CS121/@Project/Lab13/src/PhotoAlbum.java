 ...
 * @author Ahram Kim - complete getPhotoArray() method
 */
public class PhotoAlbum
{
	...
	public Photo[][] getPhotoSquare() {
		...
		int x = (int) Math.ceil(Math.sqrt(photos.size()));
		
		Photo[][] photoSquare = new Photo[x][x];
		
		for(int row=0; row<photoSquare.length; row++) {
			for(int col=0; col<photoSquare[row].length; col++) {
				photoSquare[row][col] = photos.get((col+(x*row)) % photos.size());
