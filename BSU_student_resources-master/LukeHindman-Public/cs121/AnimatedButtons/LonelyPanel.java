...
public class LonelyPanel extends JPanel{
	...
	// Instance Variables
	private LonelyButton[][] familyButtons;
	...
	private void buildGridPanel(int gridSize) {
		...
		familyButtons = new LonelyButton[numRows][numCols];
		
		/* Load buttons into array */
		GridButtonListener gbl = new GridButtonListener();
		
		for (int i=0; i < familyButtons.length; i++) {
			for (int j = 0; j < familyButtons[i].length; j++) {
				familyButtons[i][j] = new LonelyButton();
				familyButtons[i][j].addActionListener(gbl);
	...
	private class GridButtonListener implements ActionListener {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			//LonelyButton b = (LonelyButton) e.getSource();
			
			Object o = e.getSource();
			if (o instanceof LonelyButton) {
				LonelyButton b = (LonelyButton) o;
				if (b.isActive()) {
					b.setActive(false);
				} else {
					b.setActive(true);
